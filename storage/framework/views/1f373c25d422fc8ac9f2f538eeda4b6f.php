...
$__newAttributes = [];
$__propNames = \Illuminate\View\ComponentAttributeBag::extractPropNames((['title' => 'Dashboard', 'breadcrumb' => []]));
...
foreach (array_filter((['title' => 'Dashboard', 'breadcrumb' => []]), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
}
...
<header class="sticky top-0 z-30 bg-white border-b border-gray-200 px-4 sm:px-6 py-3 flex items-center justify-between min-w-0">
    ...
    <div class="flex items-center gap-3 min-w-0">
        ...
        <nav class="hidden md:flex items-center gap-1 text-sm truncate">
            <a href="<?php echo e(route('dashboard')); ?>" class="text-gray-400 hover:text-gray-600 transition-colors">
            ...
            <?php if($loop->last): ?>
                <span class="text-gray-700 font-medium truncate max-w-xs"><?php echo e($crumb['label']); ?></span>
            <?php else: ?>
                <a href="<?php echo e($crumb['url']); ?>" class="text-gray-400 hover:text-gray-600 truncate max-w-xs"><?php echo e($crumb['label']); ?></a>
            <?php endif; ?>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <?php if(empty($breadcrumb)): ?>
                <span class="text-gray-300 mx-1">/</span>
                <span class="text-gray-700 font-medium truncate"><?php echo e($title); ?></span>
            <?php endif; ?>
        </nav>

        <!-- Title fallback on mobile -->
        <span class="md:hidden text-sm font-semibold text-gray-800 truncate max-w-[150px] sm:max-w-[260px]"><?php echo e($title); ?></span>
    </div>
        ...
        <div class="relative" x-data="{ open: false }">
            <button @click="open = !open"
                ...
                <span class="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 text-xs bg-red-500 text-white rounded-full flex items-center justify-center font-semibold"
                    ...
                    <?php echo e(min($notifications['total'], 99)); ?>
            ...
            <div x-show="open" @click.away="open = false"
                ...
                <div class="divide-y divide-gray-50 max-h-72 overflow-y-auto">
                    <?php if($notifications['critical'] > 0): ?>
                    <div class="flex items-center gap-3 px-4 py-3">
                        <div class="w-2 h-2 rounded-full bg-red-500 flex-shrink-0"></div>
                        <div>
                            <p class="text-sm text-gray-800 font-medium"><?php echo e($notifications['critical']); ?> Critical Stock</p>
                            <p class="text-xs text-gray-400">Immediate action required</p>
                        </div>
                    </div>
                    <?php endif; ?>
                    <?php if($notifications['low_stock'] > 0): ?>
                    <div class="flex items-center gap-3 px-4 py-3">
                        <div class="w-2 h-2 rounded-full bg-yellow-500 flex-shrink-0"></div>
                        <div>
                            <p class="text-sm text-gray-800 font-medium"><?php echo e($notifications['low_stock']); ?> Low Stock</p>
                            <p class="text-xs text-gray-400">Below minimum threshold</p>
                        </div>
                    </div>
                    <?php endif; ?>
                    <?php if($notifications['expiring_soon'] > 0): ?>
                    <div class="flex items-center gap-3 px-4 py-3">
                        <div class="w-2 h-2 rounded-full bg-orange-500 flex-shrink-0"></div>
                        <div>
                            <p class="text-sm text-gray-800 font-medium"><?php echo e($notifications['expiring_soon']); ?> Expiring Soon</p>
                            <p class="text-xs text-gray-400">Check expiry dates</p>
                        </div>
                    </div>
                    <?php endif; ?>
                    <?php if($notifications['expired'] > 0): ?>
                    <div class="flex items-center gap-3 px-4 py-3">
                        <div class="w-2 h-2 rounded-full bg-red-700 flex-shrink-0"></div>
                        <div>
                            <p class="text-sm text-gray-800 font-medium"><?php echo e($notifications['expired']); ?> Expired</p>
                ...
                <div class="px-4 py-2 border-t border-gray-100">
                    <a href="<?php echo e(route('chemicals.index', ['status' => 'CRITICAL'])); ?>" class="text-xs text-blue-600 hover:text-blue-800 font-medium">View all alerts →</a>
                </div>
            </div>
        </div>
...
<?php /**PATH C:\xampp\htdocs\chemical 2\resources\views/components/topbar.blade.php ENDPATH**/ ?>